...
try {
    HBLogger.info(`call redisClient.connect() for host ${process.env.REDIS_HOST}`);
    redisClient.connect();
} catch (error) {
    HBLogger.error(`error connecting to redis on connection method as ${error.message} for host ${process.env.REDIS_HOST}`);
}
...
redisClient.on("error", (error) => {
    HBLogger.error(`error connecting to redis in on error method as  ${error.message} for host ${process.env.REDIS_HOST}`);
});
