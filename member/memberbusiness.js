...
memberBusiness.redirectToYogaClass = async (req, res, next) => {
    ...
    try {
        if (req && req.params && req.params.shortroute) {
            ...
            HBLogger.info(
                `memberBusiness.redirectToYogaClass call in for shortRoute: ${shortRoute}`
            );
            ...
            if (memberCacheResult) {
                HBLogger.info(
                    `memberBusiness.redirectToYogaClass call in cache for shortRoute: ${shortRoute}`
                );
                ...
                if (memberCacheResult.is_youtube) {
                    ...
                    if (yTCacheValue) {
                        ...
                        HBLogger.info(
                            `memberBusiness.redirectToYogaClass from cache Opened video with YouTube app for link: ${memberCacheResult.ytlink} for shortroute: ${shortRoute}`
                        );
                    } else {
                        const ytLinkResult =
                        ...
                        memberCacheResult.ytlink =
                            ytLinkResult && ytLinkResult.rowCount > 0
                                ? ytLinkResult.rows[0].ytlink
                                : '';
                        ...
                        HBLogger.info(
                            `memberBusiness.redirectToYogaClass get ytlink from database link: ${memberCacheResult.ytlink} for shortroute: ${shortRoute}`
                        );
                    }

                    appLink = await getYTAppLink(
                        memberCacheResult,
                        userAgent,
                        shortRoute
                    );
                    ...
                    HBLogger.info(
                        `memberBusiness.redirectToYogaClass call in cache for shortRoute: ${shortRoute} and redirected to zoom.`
                    );
                ...
                if (memberCacheResult.type == 'MEET') {
                    ...
                    res.cookie('shortRoute', shortRoute, {
                        ...
                        sameSite:
                            process.env.COOKIE_SAME_SITE == 'true'
                                ? undefined
                                : 'None',
                        maxAge:
                            3600 *
                    ...
                    addShortLinkRedirectionLog(
                        memberCacheResult,
                        shortRoute,
                        req
                    );
                } else {
                    const cookieShortRoute = req.cookies
                        ? req.cookies['shortRoute']
                        : '';
                    let cookieMemberCacheResult = await redishandler.get(
                        cookieShortRoute
                    );
                    if (cookieMemberCacheResult) {
                        HBLogger.info(
                            `memberBusiness.redirectToYogaClass log redirection in redis cache for cookie shortRoute: ${cookieShortRoute} in custom shortlink clicked by member`
                        );
                        cookieMemberCacheResult = JSON.parse(
                            cookieMemberCacheResult
                        );
                        addShortLinkRedirectionLog(
                            cookieMemberCacheResult,
                            cookieShortRoute,
                        ...
                        HBLogger.info(
                            `memberBusiness.redirectToYogaClass log redirection in redis cache completed for cookie shortRoute: ${cookieShortRoute} in custom shortlink clicked by member`
                        );
                    }
                }
            } else {
                HBLogger.info(
                    `memberBusiness.redirectToYogaClass call in database for shortRoute: ${shortRoute}`
                );
                const memberResult = await memberDa.processQueryWithFilters(
                    GET_SHORT_LINK_DETAILS_ROUTE_BY_ROUTE,
                    [shortRoute, process.env.YOGA_CLASS_PROGRAM_ID]
                );
                ...
                if (memberResult && memberResult.rowCount > 0) {
                    ...
                    if (
                        ...
                        await redishandler.set(
                            ...
                            JSON.stringify({
                                ...
                                is_youtube:
                                    member.current_channel.toLowerCase() ==
                                        'zoom' ||
                                    member.current_channel.toLowerCase() ==
                                        'custommeettype'
                                        ? false
                        ...
                        if (member.current_channel.toLowerCase() == 'zoom') {
                            ...
                            HBLogger.info(
                                `memberBusiness.redirectToYogaClass Opened video with YouTube app for link: ${appLink} for shortroute: ${shortRoute}`
                            );
                        }
                    } else {
                    ...
                    res.locals.data = commonFunctions.createResponse(
                        {},
                        { message: `short url ${shortRoute} does not exist.` }
                    );
                    next();
                }
            }
            ...
            res.locals.data = commonFunctions.createResponse(
                {},
                { message: 'Not a valid short url.' }
            );
            next();
        }
    } catch (err) {
        HBLogger.error(
            `memberBusiness.redirectToYogaClass error: ${err.message} ${err.stack}`
        );
        res.locals.data = commonFunctions.createResponse(
            {},
...
memberBusiness.redirectToTest = async (req, res, next) => {
    HBLogger.info(
        `memberBusiness.redirectToTest headers: ${JSON.stringify(
            req.headers
...
const getYTAppLink = async (
    ...
    if (
        ...
        appLink = appLink;
        HBLogger.info(
            `getYTAppLink for shortRoute: ${shortRoute} userAgent:${userAgent} and  mobile device for iPhone or ipad with redirecturl: ${appLink}`
        );
        ...
        HBLogger.info(
            `getYTAppLink for shortRoute: ${shortRoute} userAgent:${userAgent} and mobile device found with redirecturl: ${appLink}`
        );
    } else {
        HBLogger.info(
            `getYTAppLink for shortRoute: ${shortRoute} userAgent:${userAgent} and not from mobile device and redirecturl: ${appLink}`
        );
    }
    return appLink;
};
...
const addShortLinkRedirectionLog = async (member, shortRoute, req) => {
    HBLogger.info(
        `addShortLinkRedirectionLog call in for redis insert attendance shortRoute: ${shortRoute}`
    );
    const myObject = {
        id: member.id,
        shortlinkid: member.shortlinkid,
    ...
    try {
        ...
        console.log(
            `addShortLinkRedirectionLog PlanetScale Attendance db error: ${error.message}`
        );
    }
};
...
memberBusiness.redirectTestYogaMeeting = async (req, res, next) => {
    ...
    try {
        if (
            ...
            appLink = `youtube://watch?v=${videoId}`;
            HBLogger.info(
                `memberBusiness.redirectYogaMeeting ${os.platform()}`
            );
        }
        console.log(
            `Opened video with ID ${videoId} in the YouTube app for link: ${appLink}`
        );
    } catch (error) {
        console.error(
            `memberBusiness.redirectYogaMeeting Error opening video with ID ${videoId} : ${error.message}`
        );
    }
...
memberBusiness.resetRedisKey = async (req, res, next) => {
    ...
    try {
        if (req && req.params && req.params.key) {
            ...
            res.locals.data = commonFunctions.createResponse(
                { message: `short url ${req.params.key} reset successfyll.` },
                {}
            );
        } else {
            res.locals.data = commonFunctions.createResponse(
...
memberBusiness.deleteMultipleKeys = async (req, res, next) => {
    ...
    try {
        if (req && req.body && req.body.keys && req.body.keys.length > 0) {
            const lowerCaseArray = req.body.keys.map((str) =>
                str.toLowerCase()
            );

            await redishandler.DEL(lowerCaseArray);
            res.locals.data = commonFunctions.createResponse(
                { message: `short urls reset successfyll.` },
                {}
            );
        } else {
            res.locals.data = commonFunctions.createResponse(
        ...
        HBLogger.error(
            `memberBusiness.deleteMultipleKeys error: ${error.message}`
        );
    }
    ...
    try {
        if (req && req.params && req.params.shortroute) {
            ...
            if (!shortRoute) {
                ...
                HBLogger.info(
                    `memberBusiness.redirectToDanceStretchClass call in for shortRoute: ${shortRoute} got from cookies`
                );
            } else
                HBLogger.info(
                    `memberBusiness.redirectToDanceStretchClass call in for shortRoute: ${shortRoute}`
                );
            ...
            if (memberCacheResult) {
                HBLogger.info(
                    `memberBusiness.redirectToDanceStretchClass call in cache for shortRoute: ${shortRoute}`
                );
                memberCacheResult = JSON.parse(memberCacheResult);
                ...
                if (yTCacheValue) {
                    ...
                    HBLogger.info(
                        `memberBusiness.redirectToDanceStretchClass from cache Opened video with YouTube app for link: ${memberCacheResult.dsytlink} for shortroute: ${shortRoute}`
                    );
                } else {
                    ...
                    memberCacheResult.dsytlink =
                        ytLinkResult && ytLinkResult.rowCount > 0
                            ? ytLinkResult.rows[0].dsytlink
                            : '';
                    ...
                    HBLogger.info(
                        `memberBusiness.redirectToDanceStretchClass get dsytlink from database dsytlink: ${memberCacheResult.dsytlink} for shortroute: ${shortRoute}`
                    );
                }
                ...
                if (memberCacheResult.type == 'MEET') {
                    ...
                    if (cookieMemberCacheResult) {
                        HBLogger.info(
                            `memberBusiness.redirectToDanceStretchClass log redirection in redis cache for cookie shortRoute: ${cookieShortRoute} in custom shortlink clicked by member`
                        );
                        cookieMemberCacheResult = JSON.parse(
                            cookieMemberCacheResult
                        );
                        ...
                        HBLogger.info(
                            `memberBusiness.redirectToDanceStretchClass log redirection in redis cache completed for cookie shortRoute: ${cookieShortRoute} in custom shortlink clicked by member`
                        );
                    }
                }
            } else {
                HBLogger.info(
                    `memberBusiness.redirectToDanceStretchClass call in database for shortRoute: ${shortRoute}`
                );
                const memberResult = await memberDa.processQueryWithFilters(
                    GET_SHORT_LINK_DETAILS_ROUTE_BY_ROUTE,
                    [shortRoute, process.env.YOGA_CLASS_PROGRAM_ID]
                ...
                if (memberResult && memberResult.rowCount > 0) {
                    ...
                    if (
                        ...
                        HBLogger.info(
                            `memberBusiness.redirectToDanceStretchClass Opened video with YouTube app for link: ${appLink} for shortroute: ${shortRoute}`
                        );
                    } else {
                        //// Redirect to payement page. In this case long_url contains the membershipplan url.
                        appLink = member.long_url;
        ...
        HBLogger.error(
            `memberBusiness.redirectToDanceStretchClass error: ${err.message} ${err.stack}`
        );
        res.locals.data = commonFunctions.createResponse(
            {},
            { message: 'Something went wrong' }
        ...
        for (let index = 0; index < redirectResult.rows.length; index++) {
            try {
                ...
                if (element && element.useragent) {
                    ...
                    jsonArray.push({ id: element.id, device: deviceType });
                }
            } catch (err) {
                HBLogger.error(
                    `memberBusiness.calculateDevice error: ${err.message} ${err.stack}`
                );
            }
        }

        await memberDa.processQueryWithFilters(UPDATE_DEVICE_IN_REDIRECT_LOGS, [
